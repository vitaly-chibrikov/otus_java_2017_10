...
public class Main {
    ...
    private static void apacheExample(Student student, String file) throws IOException, ClassNotFoundException {
        org.apache.commons.io.FileUtils.writeByteArrayToFile(new File(file), JavaIOHelper.serialize(student));
        Student readStudent = (Student) JavaIOHelper.deserialize(FileUtils.readFileToByteArray(new File(file)));
        System.out.println(readStudent);
    }

    private static void ioExample(Student student, String file) {
        JavaIOHelper.writeObject(file, student);
        Student readStudent = (Student) JavaIOHelper.readObject(file);
        System.out.println(readStudent);
    }
